...
The key to achieve efficient matrix multiplication is to reduce the number of memory read/write operations as memory is the bottleneck of the program (rather than CPU flop rate). This principle is applied to different levels of memory (as the figure below). Intuitively, when data are loaded from RAM into cache or register, we want to perform as much computation as possible using the data before releasing them from cache or register.
<p align="center">
